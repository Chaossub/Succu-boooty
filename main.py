...
def main():
    ...
    async def _portal_home_cb(_, cq: CallbackQuery):
        """
        Central 'home' panel used by the rest of the bot.
        Includes a secret-ish Sanctuary Controls button that only
        works for the owner.
        """
        rows = [
    ...
    async def _models_elsewhere_cb(_, cq: CallbackQuery):
        ...
        kb = InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅ Back to Main", callback_data="portal:home")]]
        )
        try:
            await cq.message.edit_text(
