...
async def handle_health(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    logger.debug(f"🔍 Health-check received raw request: {data!r}")
    writer.write(
        b"HTTP/1.1 200 OK\r\n"
...
async def run_bot(stop_event: asyncio.Event):
    ...
    try:
        ...
        await app.start()
        logger.info("✅ SuccuBot started, now idle()")
        await idle()
        ...
        logger.error("🔥 Exception starting or idling SuccuBot:", exc_info=True)
