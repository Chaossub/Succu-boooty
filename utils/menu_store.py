...
class MenuStore:
    def __init__(self):
        ...
        if _MONGO_URL:
            try:
                ...
                log.info(
                    "MenuStore: Mongo OK db=%s coll=%s",
                    _MONGO_DB,
                    _MENU_COLL,
                )
            except Exception as e:
                log.warning(
                    "MenuStore: Mongo unavailable, falling back to JSON: %s", e
                )
                self._use_mongo = False
    ...
    def _load_json(self):
        try:
            ...
            log.warning("MenuStore: failed to load JSON: %s", e)
            self._cache = {}
