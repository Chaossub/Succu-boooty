...
async def _send_flyer(client: Client, job: dict):
    ...
    if flyer:
        await client.send_photo(job['chat_id'], flyer['file_id'], caption=flyer['caption'], **kwargs)
...
def register(app: Client, scheduler):
    # ─── Flyer CRUD ─────────────────────────────────
    @app.on_message(filters.command('addflyer') & filters.photo & filters.group)
    ...
    @app.on_message(filters.command('changeflyer') & filters.photo & filters.group)
    ...
    async def scheduleflyer_handler(client, message: Message):
        ...
        if name not in load_flyers(message.chat.id):
            return await message.reply(f'❌ Flyer "{name}" not found.')
        job = { 'type':'flyer','name':name,'time':time_str,'chat_id':dest,
                'origin_chat_id':message.chat.id,'thread_id':thread_id }
    ...
    async def scheduletext_handler(client, message: Message):
        ...
        if parts[3].isdigit(): thread_id, idx = int(parts[3]), 4
        text = ' '.join(parts[idx:])
        try:
            hour, minute = map(int, time_str.split(':'))
            dest = resolve_target(target)
            ...
            return await message.reply(f'❌ {e}')
        job = { 'type':'text','time':time_str,'chat_id':dest,'text':text,'thread_id':thread_id }
        data = load_scheduled() + [job]
        save_scheduled(data)
