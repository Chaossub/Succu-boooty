...
async def schedule_flyer_cmd(client: Client, message: Message):
    logger.debug("[schedule_flyer_cmd] got: %s", message.text)
    if not await is_admin(client, message.chat.id, message.from_user.id):
...
async def schedule_text_cmd(client: Client, message: Message):
    logger.debug("[schedule_text_cmd] got: %s", message.text)
    if not await is_admin(client, message.chat.id, message.from_user.id):
    ...
    for job in jobs:
        if job.get('type') == 'text':
            if 'run_date' in job:
                ...
                scheduler.add_job(_send_text, trigger='date', run_date=run_date, args=[app, job])
            else:
            ...
            if 'run_date' in job:
                run_date = datetime.fromisoformat(job['run_date'])
                scheduler.add_job(_send_flyer, trigger='date', run_date=run_date, args=[app, job])
            else:
                h, m = map(int, job['time'].split(':'))
