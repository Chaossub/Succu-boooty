# req_store.py
# Manual requirements tracker (JSON-based, month-scoped) for SuccuBus Sanctuary
# Requirement: $20 OR 4+ games. Tracks tip_count and dollars. Supports one free-pass (until YYYY-MM).
# Safe to use alongside existing code (no imports from old modules).
...
REQUIREMENT_GAMES = 4
DATA_DIR = os.getenv("REQ_DATA_DIR", "data/requirements")  # change via env if you like

os.makedirs(DATA_DIR, exist_ok=True)

# --- reminder rotation state (persistent across restarts) ---
...
def next_reminder_index(n_templates: int) -> int:
    """Returns the next template index in a rotating sequence and saves it."""
    st = _load_rem_state()
...
def _user(doc: Dict[str, Any], user_id: int) -> Dict[str, Any]:
    ...
    if uid not in doc["users"]:
        doc["users"][uid] = {
            "user_id": user_id,
            "tips_usd": 0.0,
            "tip_count": 0,          # number of tips this month
            "games": 0,
            "met": False,
...
def set_pass(user_id: int, until_month: str, month: Optional[str] = None) -> Dict[str, Any]:
    doc = ensure_month(month)
    u = _user(doc, user_id)
    u["free_pass_until"] = until_month  # "YYYY-MM"
    _recompute(u)
...
def list_behind(month: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for u in doc["users"].values():
        if not u.get("met") and not _has_active_pass(u):
            behind.append(u)
    return behind
...
def list_met(month: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    good = []
    for u in doc["users"].values():
        if u.get("met") or _has_active_pass(u):
            good.append(u)
    return good
...
def export_csv(path: str, month: Optional[str] = None) -> str:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["user_id", "tips_usd", "tip_count", "games", "met", "free_pass_until", "notes"])
        for u in doc["users"].values():
            w.writerow([u["user_id"], u.get("tips_usd",0.0), u.get("tip_count",0), u.get("games",0),
...
def clear_current_month() -> str:
    """Completely clears the current month's JSON file (resets users to empty)."""
    m = month_key()
    doc = {"month": m, "users": {}, "audit": [{"ts": datetime.utcnow().isoformat(), "op": "clear_month"}]}
    _save(doc, m)
    # Also reset reminder rotation to start fresh
