...
def _compute_targets(action: str) -> List[Dict[str, Any]]:
    ...
    for md in docs:
        if md.get("is_exempt") or md.get("removed"):
            continue

        if action == "reminder" and md.get("met"):
            continue
        if action == "final" and md.get("met"):
            continue

        targets.append(md)

    def _sort_key(m: Dict[str, Any]):
        return (m.get("username") or "", m.get("first_name") or "", str(m.get("user_id") or ""))
