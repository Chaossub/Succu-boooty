...
try:
    ...
    # Fallback if tzdata is missing in the host environment.
    # (This will not perfectly handle DST, but keeps booking functional.)
...
# ────────────── Availability from Admin Panel (NSFW_AVAIL:<yyyymmdd>) ──────────────
...
def _load_day_avail(date_iso: str) -> dict:
    # date_iso: YYYY-MM-DD
    yyyymmdd = date_iso.replace("-", "")
    raw = store.get_menu(_avail_key(yyyymmdd)) or ""
    d = _jloads(raw, {}) if raw else {}
    # expected shape: {"on": bool, "windows": [[HH:MM,HH:MM],..], "blocks": [[HH:MM,HH:MM],..]}
...
def _subtract_ranges(open_ranges, blocks):
    # open_ranges, blocks: list[(start_min,end_min)]
    for bs, be in blocks:
        new = []
        for os_, oe in open_ranges:
            if be <= os_ or bs >= oe:
                new.append((os_, oe))
            else:
                if os_ < bs:
        ...
        open_ranges = new
    # clean
    open_ranges = [(s, e) for (s, e) in open_ranges if e > s]
    open_ranges.sort()
    return open_ranges

def _effective_open_ranges(date_iso: str):
    """
    Returns (is_on, open_ranges_minutes, windows_used, blocks_used)
    open_ranges_minutes is post-block subtraction.
    """
    day = _load_day_avail(date_iso)
    ...
    if windows:
        ...
        for w in windows:
            try:
                s, e = w
                base.append((_hhmm_to_min(s), _hhmm_to_min(e)))
            except Exception:
                continue
    else:
        # fallback to default business hours
        start_str, end_str = _hours_for_date_fallback(date_iso)
        base = [(_hhmm_to_min(start_str), _hhmm_to_min(end_str))]

    b = []
    for blk in blocks:
        try:
            s, e = blk
            b.append((_hhmm_to_min(s), _hhmm_to_min(e)))
        except Exception:
            continue

    eff = _subtract_ranges(base, b) if b else base
    return True, eff, windows, blocks

def _availability_line(date_iso: str) -> str:
    is_on, ranges, windows, blocks = _effective_open_ranges(date_iso)
    if not is_on:
        return "⛔ Not available this day."
    if not ranges:
        return "⛔ Blocked all day."
    # show compact summary of available windows (post-blocks)
    parts = []
    for s, e in ranges[:4]:
        parts.append(f"{_min_to_hhmm(s)}–{_min_to_hhmm(e)}")
    more = " …" if len(ranges) > 4 else ""
    base = "Available: " + ", ".join(parts) + more + f" ({TZ_LABEL})"
    if windows and blocks:
        base += "  (blocks applied)"
    return base


def _hours_for_date(date_iso: str) -> Tuple[str, str]:
    is_on, ranges, _, _ = _effective_open_ranges(date_iso)
    if (not is_on) or (not ranges):
        # fallback to business hours for label, even if blocked/off
        return _hours_for_date_fallback(date_iso)
    start_m = min(s for s, _ in ranges)
    end_m = max(e for _, e in ranges)
    return _min_to_hhmm(start_m), _min_to_hhmm(end_m)
...
def _hours_for_date_fallback(date_iso: str) -> Tuple[str, str]:
...
def _generate_slots(date_iso: str, dur_minutes: int) -> List[str]:
    """
    Return list of HHMM strings for start times every 30 minutes within admin-set availability.
    Availability comes from NSFW_AVAIL:<yyyymmdd> (windows minus blocks) in LA time.
    If no admin windows exist, falls back to BUSINESS_HOURS.
    """
    is_on, ranges, _, _ = _effective_open_ranges(date_iso)
    if not is_on or not ranges:
        return []
...
def register(app: Client):
    ...
    async def pick_date(_, cq: CallbackQuery):
        ...
        hours_line = _availability_line(date_iso)
    ...
    async def pick_period(_, cq: CallbackQuery):
        ...
        # compute slots here so we can show a friendly message if the day is blocked
        dur_minutes = DURATIONS[dur_key]
        slots = _generate_slots(date_iso, dur_minutes)
        slots = _filter_slots_by_period(slots, period_key)
        if not slots and period_key != "all":
        ...
        if not slots:
            await cq.message.edit_text(
                f"🗓 <b>{pretty}</b>\n\n"                f"{period_label} • {_dur_label(dur_key)}\n\n"                "⛔ No times are available for this day.\n\n"                "Try another date or adjust availability.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("⬅ Back", callback_data=f"nsfw:book:dur:{date_iso}:{week_offset}:{dur_key}")],
                    [InlineKeyboardButton("📅 Pick another date", callback_data=f"nsfw:book:datepage:{week_offset}")],
                    [InlineKeyboardButton("⬅ Back to Roni Assistant", callback_data="roni_portal:home")],
                ]),
                disable_web_page_preview=True,
