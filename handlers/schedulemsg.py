...
def resolve_group_name(group: str) -> str:
    ...
    Accept:
      - raw id: '-100123...'
      - @username
      - ENV key like SUCCUBUS_SANCTUARY, MODELS_CHAT, etc.
    """
...
def set_main_loop(loop):
    """Called from main.py so jobs can post back to Telegram."""
    global MAIN_LOOP
    MAIN_LOOP = loop
...
async def schedulemsg_handler(client, message):
    ...
    # Photo logic (this message or reply target)
    photo = None
    if message.photo:
    ...
    if len(args) < 5 and not photo:
        return await message.reply(
            ...
            "Example:\n"
            "/schedulemsg 2025-07-20 18:30 SUCCUBUS_SANCTUARY Hello!\n"
...
async def listmsgs_handler(client, message):
    ...
    for msg_id, job in SCHEDULED_MSGS.items():
        ...
        lines.append(
            f"• <b>{group}</b> at <i>{run_time}</i> — <code>{msg_id}</code>"
        )
