...
from pyrogram import Client, filters  # <-- this import fixes the NameError
...
async def _unset_dmready_and_notify(client: Client, user: User, chat_id: int, reason: str):
    ...
    try:
        removed = _dm.unset_dm_ready_global(user.id) if hasattr(_dm, "unset_dm_ready_global") else _dm.clear(user.id)  # JSON store compat
    except Exception:
        removed = False
...
def register(app: Client):
    ...
    # Telegram service “new chat members”
    ...
    # Telegram service “left chat member”
    @app.on_message(filters.group & filters.left_chat_member)
    async def on_service_left(client: Client, m: Message):
        # Only enforce for configured sanctuary groups if provided
