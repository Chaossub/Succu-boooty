# handlers/welcome.py
# Group welcomes + goodbyes + DM-ready auto-remove when members leave Sanctuary groups.
...
from utils.dmready_store import global_store as store  # <-- NEW
...
# Sanctuary groups (comma/space separated ids, e.g. "-100123,-100456")
...
SANCTUARY_GROUP_IDS = _parse_ids(os.getenv("SANCTUARY_GROUP_IDS", ""))  # <-- used for auto-remove
...
def _maybe_remove_dm_ready(chat_id: int, user: Optional[User]):
    """If this chat is a Sanctuary group, remove DM-ready for this user."""
    if not user:
        return
    if chat_id in SANCTUARY_GROUP_IDS:
        removed = store.remove(user.id)
        if removed:
            # optional: you could notify OWNER_ID here if you like
            pass
...
def register(app: Client):
    ...
    async def on_service_left(client: Client, m: Message):
        ...
        if _seen(m.chat.id, u.id, "leave"):
            return
        _maybe_remove_dm_ready(m.chat.id, u)          # <-- remove on service leave
    ...
    async def on_member_updated(client: Client, upd: ChatMemberUpdated):
        ...
        if new.status in (ChatMemberStatus.LEFT, ChatMemberStatus.BANNED):
            if _seen(chat.id, user.id, "leave"):
                return
            # Remove DM-ready if this is a Sanctuary group
            _maybe_remove_dm_ready(chat.id, user)     # <-- remove on leave/kick/ban
