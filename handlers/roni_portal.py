...
def register(app: Client) -> None:
    ...
    # Menu (supports optional src marker)
    ...
    async def roni_age_cb(_, cq: CallbackQuery):
        # roni_portal_age.py handles the actual verification
        await cq.answer()
    ...
    async def roni_admin_cancel_cb(_, cq: CallbackQuery):
        ...
        await cq.message.edit_text("Cancelled 💜", reply_markup=_roni_main_keyboard(cq.from_user.id), disable_web_page_preview=True)
        await cq.answer()
    ...
    async def age_list(_, cq: CallbackQuery):
        if cq.from_user.id != RONI_OWNER_ID:
            await cq.answer("Only Roni 💜", show_alert=True)
            return

        # lightweight: show count and a note (full list is in roni_portal_age.py)
        await cq.message.edit_text(
            "✅ <b>Age-Verified List</b>\n\n"
            "Use the age verification handler to view the full list.\n\n"
            "⬅ Tap back to return.",
