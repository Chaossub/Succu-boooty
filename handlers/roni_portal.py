...
def register(app: Client) -> None:
    ...
    # ✅ FIX: run early so other /start handlers don't swallow it
    ...
    # ✅ Menu supports "came from booking" so it can show "Back to Booking" ONLY then
    @app.on_callback_query(filters.regex(r"^roni_portal:menu(?::src=nsfw)?$"))
    async def roni_menu_cb(_, cq: CallbackQuery):
        came_from_nsfw = (cq.data == "roni_portal:menu:src=nsfw")
    ...
    async def roni_teaser_cb(_, cq: CallbackQuery):
        ...
        if user_id != RONI_OWNER_ID and not (user_id and is_age_verified(user_id)):
            await cq.answer("You’ll need to complete age verification before seeing Roni’s teaser channels. 💕", show_alert=True)
            return

        teaser_text = store.get_menu(TEASER_TEXT_KEY) or (os.getenv("RONI_TEASER_CHANNELS_TEXT") or "Roni will add her teaser & promo channels here soon. 💕")
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back to Roni Assistant", callback_data="roni_portal:home")]])
    ...
    async def roni_admin_edit_menu_cb(_, cq: CallbackQuery):
        ...
        store.set_menu(f"_RONI_PENDING:{cq.from_user.id}", "menu")
    ...
    async def roni_admin_edit_open_cb(_, cq: CallbackQuery):
        ...
        store.set_menu(f"_RONI_PENDING:{cq.from_user.id}", "open_access")
    ...
    async def roni_admin_edit_teaser_cb(_, cq: CallbackQuery):
        ...
        store.set_menu(f"_RONI_PENDING:{cq.from_user.id}", "teaser")
        current = store.get_menu(TEASER_TEXT_KEY) or (os.getenv("RONI_TEASER_CHANNELS_TEXT") or "No teaser/promo text set yet.")
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="roni_admin:cancel")]])
        await cq.message.edit_text(
    ...
    async def roni_admin_edit_sanctuary_cb(_, cq: CallbackQuery):
        ...
        store.set_menu(f"_RONI_PENDING:{cq.from_user.id}", "sanctuary")
