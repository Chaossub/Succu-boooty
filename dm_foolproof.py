# dm_foolproof.py
# Private /start portal + DM-ready tracking (persists via dmready_store).
...
def register(app: Client):
    ...
    async def start_portal(client: Client, m: Message):
        """Mark user DM-ready (once), notify OWNER, and show the portal."""
        u = m.from_user
        if not u:
            return

        # Mark ready once (persists to JSON)
